...
const JobListings = () => {
  ...
  const [sortField, setSortField] = useState<"datePosted" | "salary">(
    "datePosted"
  );
  ...
  const parseJobListings = (text: string): JobListing[] => {
        ...
        if (match) {
          ...
          try {
            ...
            if (jobsArray.length > 0) {
              return jobsArray
                .filter(
                  (job: { title: any; company: any; description: any }) =>
                ...
                .map((job: any, index: number) => ({
                  ...
                  company:
                    ...
                    "Unknown Company",
                  location:
                    job.location || job.city || job.job_location || "Remote",
                  salary:
                    job.salary || job.compensation || job.salary_range || null,
                  datePosted:
                    job.datePosted ||
                    job.posted_date ||
                    job.date_posted ||
                  ...
                  description:
                    job.description || job.summary || job.job_description || "",
    ...
    sections.forEach((section) => {
      ...
      const titleMatch =
        section.match(/(?:Position|Title|Job|Role)[\s:]+([^\n]+)/i) ||
        section.match(/^\d+\.\s*([^-\n]+?)(?:\s*-|$)/);
      const companyMatch =
        section.match(/(?:Company|Employer|Organization|At)[\s:]+([^\n]+)/i) ||
        section.match(/(?:at|@)\s+([^\n,]+)/i);
      const locationMatch = section.match(
        /(?:Location|City|Where|Based)[\s:]+([^\n]+)/i
      );
      const salaryMatch =
        section.match(/(?:Salary|Compensation|Pay|Range)[\s:]+([^\n]+)/i) ||
        section.match(/\$[\d,]+(?:\s*-\s*\$?[\d,]+)?(?:k)?/i);

      if (titleMatch && (companyMatch || section.includes("Company"))) {
        const title = titleMatch[1].trim();
        if (!title || title.length < 3 || title.toLowerCase().includes("level"))
          return;
  ...
  const displayJobs =
    useAgentJobs && agentJobs.length > 0 ? agentJobs : mockJobListings;

  const handleResearch = (job: JobListing) => {
    // navigate to company research page for this job
    navigate(`/company-research/${job.id}`);
  };

  const handleFavoriteToggle = async (job: JobListing) => {
    // toggle favorite locally
    const updated = displayJobs.map((j) =>
      j.id === job.id ? { ...j, favorite: !j.favorite } : j
    );
    setUseAgentJobs(false); // force re-render using mock or agent jobs
    setAgentJobs(updated); // override for agent jobs
    // update Firestore applications array
    if (user?.uid) {
  ...
  const filteredJobs = displayJobs
    .filter((job) => {
      if (
        ...
        return false;
      if (locationFilter.length && !locationFilter.includes(job.location))
        return false;
      if (statusFilter.length && !statusFilter.includes(job.status))
        return false;
      return true;
    })
    .sort((a, b) => {
  ...
  useEffect(() => {
    if (showChatBot && chatMessages.length === 0 && !loading) {
      const initialMessage = `Hello! Please return ONLY a JSON object with job listings in this format:
{
  "jobs": [
    {
      "title": "Job Title",
  ...
  const chatBotDescription = `Connected to Speaker Agent (User: ${
    user?.uid?.substring(0, 8) || "anonymous"
  }...)`;
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">
            Job Listings{" "}
            {useAgentJobs && agentJobs.length > 0 && "(AI Results)"}
          </h1>
      ...
      {useAgentJobs && agentJobs.length > 0 && (
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          <p className="text-sm text-blue-800 dark:text-blue-200">
            Showing {agentJobs.length} jobs found by the AI assistant. Ask the
            assistant to find more specific jobs based on your preferences!
          </p>
        </div>
      )}
